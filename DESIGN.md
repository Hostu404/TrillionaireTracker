...
**A regional AIS source (Kystverket) was tried and removed, 2026-09-23.** A second, independent listener against Kystverket's (the Norwegian Coastal Administration's) public real-time AIS relay — a raw NMEA/AIVDM TCP stream, no API key needed — was added alongside aisstream.io's global feed, on the reasoning that free supplementary coverage for Scandinavian waters was worth having even though it'd only ever help a narrow slice of the tracked roster. The AIVDM bit-decoding itself was verified two ways (a self-consistent round-trip test, and an exact match against three third-party `pyais` test fixtures) before being wired in, and it was built to fail safely — a broad `except Exception` around the whole listener so it could never take the aisstream.io side down with it. That defensive design was then confirmed for real: a live GitHub Actions run (`.github/workflows/ais.yml` #231) showed the TCP connection itself getting refused from that environment (`[Errno 110] Connect call failed`), caught and logged exactly as intended, with the job still completing successfully and aisstream.io unaffected. But "fails safely" isn't the same as "delivers value" — this project's actual deployment (GitHub Actions burst mode, the documented "recommended default") got zero benefit from it and, per that same `Errno 110` (a timeout pattern more consistent with a structural network block than a one-off hiccup), likely never would. Given that, plus the real added complexity of an AIVDM decoder in an otherwise simple stdlib backend, it was pulled rather than kept as permanently-dead code. It could be revisited under Option B (a continuously-running listener on a normal home/VPS network, which was never actually tested and might reach Kystverket fine) if Norwegian-waters coverage ever becomes worth the effort again.
