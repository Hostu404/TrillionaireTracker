...
**Option A — free, no server (recommended default).** `backend/ais_listener.py` supports a burst mode: set `AIS_BURST_SECONDS`, and it connects, subscribes, listens for that window, saves whatever came in, and exits — no always-on process needed. `.github/workflows/ais.yml` runs this on the same free 5-minute Actions cron as `snapshot.yml`, reading the `AISSTREAM_API_KEY` repo secret (free at aisstream.io/authenticate). This catches only whatever a vessel transmits during the listening window — currently 270 seconds (`AIS_BURST_SECONDS`, raised from an initial 45s) — so think "refreshed every 5 minutes," not instant. `snapshot_worker.py` only ever reads the resulting `ais_cache.json` — zero extra network calls of its own.

As of 2026-09-23, a missing `AISSTREAM_API_KEY` secret no longer skips the run entirely: `ais_listener.py` also runs the Kystverket regional listener (see below), which needs no key at all, so the workflow now always invokes the script and lets it decide internally what it can run — aisstream.io's global coverage if the key is set, Kystverket's Norway-only coverage either way. The step just logs a warning instead of exiting early when the key's absent.
...
**A second, regional AIS source — Kystverket — added 2026-09-23.** Alongside aisstream.io's global-but-gappy WebSocket feed, `ais_listener.py` now also runs a second, independent listener against Kystverket's (the Norwegian Coastal Administration's) public real-time AIS relay — a raw NMEA/AIVDM TCP stream (`153.44.253.27:5631`) that needs no API key, no registration, and no new dependency (plain stdlib `asyncio.open_connection`). Its tradeoff is coverage, not reliability: it only hears vessels within range of Norwegian coastal base stations, so it's a real but narrow supplement — useful specifically when one of the tracked yachts cruises Scandinavian waters, silent otherwise, and purely additive (a quiet run changes nothing from before this existed). Parsing raw AIVDM sentences (6-bit-ASCII-armored NMEA, needing its own bit-level decode of AIS Message Types 1/2/3 for position/speed/course) is real complexity with no precedent elsewhere in this stdlib-only backend, so it was verified two ways before being wired in: a self-consistent round-trip test (encode known values with a matching test-only encoder, decode them back, confirm an exact match, across six cases including antimeridian/equator edge cases), and — since that alone only proves internal consistency, not correctness against the real wire format — a check against three sentences from `pyais`'s own published test suite (a widely-used third-party AIS decoding library whose fixtures are themselves hand/online-decoder-verified per that project's own test comments), matching exactly on MMSI, latitude, longitude, and speed for every fixture tried. What that verification can't cover, because this sandbox has no network route to arbitrary IPs: an actual TCP session against Kystverket's real server. Same caveat this file already carries for the aisstream.io side (see the Known gaps entry on it) — watch the logs the first time this runs for real. Both listeners write into the same `ais_cache.json` concurrently without any lock; see `_kystverket_listen_once`'s docstring in `ais_listener.py` for why that's safe (the read-modify-save cache sequence never has an `await` in the middle, so asyncio can't interleave it with the other listener's own sequence). `AISSTREAM_API_KEY` is no longer strictly required to run `ais_listener.py` at all — missing it now just means skipping aisstream.io's global coverage for Kystverket's regional-only one, rather than refusing to start.
...
7. For vessels, add an `AISSTREAM_API_KEY` repo secret (free at aisstream.io/authenticate → Settings → Secrets and variables → Actions → New repository secret) for global AIS coverage. As of 2026-09-23 this is no longer strictly required — `.github/workflows/ais.yml` also runs a keyless regional listener (Kystverket, Norwegian coastal waters only — see Vessels above) — but without the secret you're relying on that narrower regional coverage alone, and most vessels will still just read as no signal.
  ...
  refresh:
    ...
    steps:
      ...
      - name: Burst-listen for AIS updates
        ...
        run: |
          # ais_listener.py itself now handles a missing AISSTREAM_API_KEY —
          # it logs a warning and runs the keyless Kystverket regional
          # listener alone (see Vessels above) rather than refusing to
          # start. This step used to exit 0 here without running anything
          # at all when the key was unset, which would have silently
          # skipped Kystverket too; it no longer does.
          if [ -z "$AISSTREAM_API_KEY" ]; then
            echo "AISSTREAM_API_KEY repo secret not set — running with the" \
                 "Kystverket regional feed only. Get a free key at" \
                 "https://aisstream.io/authenticate for global coverage too."
          fi
...
- The Kystverket regional AIS listener added 2026-09-23 (see Vessels above) has **not** been run against the real feed — this sandbox has no network route to arbitrary IPs, only to the handful of allowlisted hosts everything else here talks to. The AIVDM bit-decoding itself is checked two independent ways (a self-consistent round-trip test, and an exact match against three third-party `pyais` test fixtures — see Vessels above), so the parsing math is on solid footing; what's unverified is the socket-handling code around it — actually connecting to `153.44.253.27:5631`, how it behaves under real traffic volume from Norwegian coastal waters, and whether the burst window (currently the same `AIS_BURST_SECONDS` used for aisstream.io) is a sensible length for this source too. Same posture this file already takes with a new, not-yet-live-tested piece: shipped because the reasoning and the parts that can be tested check out, flagged here so the first real run gets watched rather than assumed clean.
